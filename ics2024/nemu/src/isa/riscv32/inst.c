...
static vaddr_t *csr_register(word_t imm){
  switch(imm){
    ...
    default: panic("Unknown csr detected.");
  }
}
...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("0000001 ????? ????? 001 ????? 01100 11", mulh   , R, sword_t a = src1; sword_t b = src2; int64_t tmp = (int64_t)a * (int64_t)b; R(rd) = BITS(tmp, 63, 32));
  ...
  INSTPAT("? ?????????? ? ???????? ????? 11011 11", jal    , J, R(rd) = s->pc + 4; s->dnpc = s ->pc + imm; IFDEF(CONFIG_ITRACE, {if(rd == 1) trace_func_call(s->pc, s->dnpc, false);}));
  ...
  INSTPAT("??????? ????? ????? 000 ????? 11001 11", jalr   , I, R(rd) = s -> pc + 4; s -> dnpc = (src1 + imm) & ~1; IFDEF(CONFIG_ITRACE, {if(s->isa.inst.val == 0x00008067){trace_func_ret(s->pc);}else if(rd == 1){trace_func_call(s->pc, s->dnpc, false);}else if(rd == 0 && imm == 0){trace_func_call(s->pc, s->dnpc, true);}}));
