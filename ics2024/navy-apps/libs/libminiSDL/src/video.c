...
static inline SDL_Rect *SDL_RectIntersect(SDL_Rect *a, SDL_Rect *b) {
  ...
  int x = MAX(a->x, b->x);
  int y = MAX(a->y, b->y);
  int xt = MIN(a->x + a->w, b->x + b->w);
  int yt = MIN(a->y + a->h, b->y + b->h);
  a->x = x;
  a->y = y;
  a->w = xt - x;
  a->h = yt - y;
  ...
  if(dst->format->BitsPerPixel == 8) {
    ...
    for(int i = 0; i < dstrect->h; i++) {
      for(int j = 0; j < dstrect->w; j++) {
        ((uint32_t *)dst->pixels)[(dstrect->y + i) * dw + dstrect->x + j] = ((uint32_t *)src->pixels)[(srect.y + i) * sw + srect.x + j];
      }
    }
  }
...
void SDL_FillRect(SDL_Surface *dst, SDL_Rect *dstrect, uint32_t color) {
  ...
  for(int i = 0; i < r->h; i++) {
    for(int j = 0; j < r->w; j++) {
      if(dst->format->BitsPerPixel == 8) {
        ((uint8_t *)dst->pixels)[(r->y + i) * w + r->x + j] = color;
      }
      else {
        ((uint32_t *)dst->pixels)[(r->y + i) * w + r->x + j] = color;
      }
    }
  }
}
...
void SDL_UpdateRect(SDL_Surface *s, int x, int y, int w, int h) {
  ...
  if(s->format->BitsPerPixel == 8) {
    ...
    for(int i = 0; i < h; i++) {
      for(int j = 0; j < w; j++) {
        gbPixels[y + i * w] = colorARGB[((uint8_t *)s->pixels)[(y + i) * s->w + x + j]];
      }
    }
