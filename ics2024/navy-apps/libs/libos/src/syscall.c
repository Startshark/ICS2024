...
void *_sbrk(intptr_t increment) {
  extern char _end;
  static intptr_t program_break = (intptr_t)&_end; // program break is a global variable
  if(_syscall_(SYS_brk, program_break + increment, 0, 0) == 0) { // if the syscall is successful
    intptr_t old_program_break = program_break;
    program_break += increment;
    return (void *)old_program_break; // return the previous program break
  }
  else return (void*) -1; // return -1 if the syscall fails 
}
